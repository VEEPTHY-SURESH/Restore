...
import { useFetchAddressQuery, useUpdateUserAddressMutation } from "../account/acountApi";
...
export default function CheckoutStepper() {
    ...
    const fetchResult = useFetchAddressQuery();
    const data = fetchResult.data ?? {} as Address;
    const { name, ...restAddress } = data;
    const isLoading = fetchResult.isLoading;
    const [updateAddress] = useUpdateUserAddressMutation();
    const [saveAddessChecked, setSaveAddessChecked] = useState(false);
    const elements = useElements();
    const stripe = useStripe();
    const [addressComplete, setAddressComplete] = useState(false);
    const [paymentComplete, setPaymentComplete] = useState(false);
    const [submitting, setSubmitting] = useState(false);
    const {basket, subtotal, deliveryFee, clearBasket} = useBasket();
    ...
    const handleNext = async () => {
        if(activeStep===0 && saveAddessChecked && elements){
            ...
            <Box sx={{mt: 2}}>
                <Box sx={{display: activeStep ===0 ? 'block' : 'none'}}>
                    ...
                    <FormControlLabel
                        ...
                        control={<Checkbox
                                checked={saveAddessChecked}
                                onChange={ e => setSaveAddessChecked(e.target.checked)}
                            />}
